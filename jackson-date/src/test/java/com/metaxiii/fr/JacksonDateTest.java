...
class JacksonDateTest {
  ...
  void whenSerializingDateWithJackson_thenSerializedToTimestamp() throws JsonProcessingException, ParseException {
    ...
    assertEquals("{\"name\":\"party\",\"eventDate\":3600000}", mapper.writeValueAsString(event));
  }
  ...
  void whenSerializingDateToISO8601_thenSerializedToText() throws JsonProcessingException, ParseException {
    ...
    assertEquals("{\"name\":\"party\",\"eventDate\":\"1970-01-01T02:30:00.000+00:00\"}", result);
  }
  ...
  void whenSettingObjectMapperDateFormat_thenCorrect() throws JsonProcessingException, ParseException {
    ...
    String result = mapper.writeValueAsString(event);
    assertEquals("{\"name\":\"party\",\"eventDate\":\"20-12-2014 02:30\"}", result);
  }
  ...
  void whenUsingJsonFormatAnnotationToFormatDate_thenCorrect() throws JsonProcessingException, ParseException {
    ...
    ObjectMapper mapper = new ObjectMapper();
    String result = mapper.writeValueAsString(event);
    assertEquals("{\"name\":\"party\",\"eventDate\":\"20-12-2014 02:30:00\"}", result);
  }

  @Test
  void whenUsingCustomDateSerializer_thenCorrect() throws JsonProcessingException, ParseException {
    SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy hh:mm:ss");
    String toParse = "20-12-2014 02:30:00";
    Date date = df.parse(toParse);
    EventWithCustomSerializer event = new EventWithCustomSerializer("party", date);
    ObjectMapper mapper = new ObjectMapper();
    String result = mapper.writeValueAsString(event);
    assertEquals("{\"name\":\"party\",\"eventDate\":\"20-12-2014 02:30:00\"}", result);
  }
  ...
  private static class CustomDateSerializer<T extends Date> extends StdSerializer<T> {
    ...
    private CustomDateSerializer(Class<T> t) {
      super(t);
    }
