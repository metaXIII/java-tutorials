...
class ItemControllerTest {
    ...
    void getItemInternal() throws Exception {
        final MvcResult mvcResult =
                mockMvc.perform(get("/items-internal").contentType(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk()).andReturn();
    ...
    void getItemPublic() throws Exception {
        final MvcResult mvcResult =
                mockMvc.perform(get("/items-public").contentType(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk()).andReturn();
        final Item item = new ObjectMapper().readValue(mvcResult.getResponse().getContentAsString(), Item.class);
        assertEquals(1, item.getId());
        assertEquals("item name", item.getItemName());
