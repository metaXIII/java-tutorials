...
class SortingArrayListUsingStreamUnitTest {
  ...
  private static final Map<String, Map<String, List<Employee>>>
      MAP_OF_DEPT_TO_MAP_OF_SEX_TO_EMPLOYEES = new HashMap<>();
  ...
  void givenHashMapContainingEmployeeList_whenSortWithStreamAPI_thenSort() throws IOException {
    ...
    MAP_OF_DEPT_TO_MAP_OF_SEX_TO_EMPLOYEES.forEach(
        (dept, deptToSexToEmps) ->
            deptToSexToEmps.forEach(
                (sex, emps) -> {
  ...
  private static String getFilePath(final String fileName) {
    return Objects.requireNonNull(
            SortingArrayListUsingStreamUnitTest.class.getClassLoader().getResource(fileName))
        .getPath();
  }
    ...
    Arrays.asList(lines)
        .forEach(
            e -> {
              ...
              final Employee emp =
                  new Employee(strArr[1], Integer.parseInt(strArr[2]), strArr[0], strArr[3]);
